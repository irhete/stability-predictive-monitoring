...
l2_3 = LSTM(lstmsize, implementation=2, kernel_initializer='glorot_uniform', return_sequences=False, dropout=dropout)(b1) # the layer specialized in outcome prediction
b2_3 = BatchNormalization()(l2_3)
outcome_output = Dense(2, activation='softmax', kernel_initializer='glorot_uniform', name='outcome_output')(b2_3)
...
if "bpic2011" in dataset_name:
    dt_train, dt_val = dataset_manager.split_val(dt_train, val_ratio=0.2, split="random")
    print(dt_train.shape, dt_val.shape)
    print(len(dt_train.groupby(dataset_manager.case_id_col)), len(dt_val.groupby(dataset_manager.case_id_col)))
    history = model.fit_generator(dataset_manager.data_generator(dt_train, max_len),
                                  validation_data=dataset_manager.data_generator(dt_val, max_len),
                                  verbose=2, callbacks=[early_stopping, model_checkpoint, lr_reducer],
                                  steps_per_epoch=len(dt_train.groupby(dataset_manager.case_id_col)), 
                                  validation_steps=len(dt_val.groupby(dataset_manager.case_id_col)), epochs=nb_epoch)
else:
