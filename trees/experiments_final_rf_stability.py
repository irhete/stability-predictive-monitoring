...
cls_method = argv[5]
results_dir = argv[6]
n_estimators = int(argv[7])
max_features = float(argv[8])
max_depth = int(argv[9])
...
cls_args = {'n_estimators': n_estimators, 'max_depth': max_depth, 'max_features': max_features}
cls_params_str = "_".join([str(val) for param, val in cls_args.items()])
cls_args['random_state'] = random_state

outfile = os.path.join(home_dir, results_dir, "final_results_%s_%s_%s_%s.csv"%(cls_method2, method_name, dataset_ref, cls_params_str)) 
detailed_results_file = os.path.join(home_dir, detailed_results_dir, "detailed_results_%s_%s_%s_%s.csv"%(cls_method2, method_name, dataset_ref, cls_params_str)) 
...
with open(outfile, 'w') as fout:
    ...
    fout.write("%s;%s;%s;%s;%s;%s;%s\n"%("dataset", "method", "cls", "params", "nr_events", "metric", "score"))
    ...
    for dataset_name in datasets:
        ...
        for nr_events in range(min_prefix_length, max_prefix_length+1):
            ...
            for bucket in set(bucket_assignments_test):
                ...
                case_ids = dataset_manager.get_case_ids(dt_test_bucket, nr_events)
                current_results = pd.DataFrame({"dataset": dataset_name, "cls": cls_method2, "params": method_name, "nr_events": nr_events, "predicted": preds_bucket, "actual": test_y_bucket, "case_id": case_ids, "params": cls_params_str})
            ...
            fout.write("%s;%s;%s;%s;%s;%s;%s\n"%(dataset_name, method_name, cls_method2, cls_params_str, nr_events, "auc", auc))
            fout.write("%s;%s;%s;%s;%s;%s;%s\n"%(dataset_name, method_name, cls_method2, cls_params_str, nr_events, "precision", prec))
            fout.write("%s;%s;%s;%s;%s;%s;%s\n"%(dataset_name, method_name, cls_method2, cls_params_str, nr_events, "recall", rec))
            fout.write("%s;%s;%s;%s;%s;%s;%s\n"%(dataset_name, method_name, cls_method2, cls_params_str, nr_events, "fscore", fscore))
